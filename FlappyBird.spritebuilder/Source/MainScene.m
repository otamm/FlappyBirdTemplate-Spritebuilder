...
@implementation MainScene {
    ...
    CCNode *_cloud1; // referenced in SpriteBuilder
    CCNode *_cloud2; // starts at the end of cloud1, creating a continuous background.
    NSArray *_clouds;
...
- (void)update:(CCTime)delta
{
    ...
    for (CCNode *bush in _bushes) {
        ...
        // if the left corner is one complete width off the screen,
        // move it to the right
        if (bush.position.x <= (-1 * bush.contentSize.width)) { // gives the impression of continuous bushes appearing
            bush.position = ccp(bush.position.x +
    ...
    for (CCNode *cloud in _clouds) { // gives the impression of continuous clouds appearing
        // move the cloud
